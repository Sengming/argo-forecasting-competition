...
import utils.baseline_utils_modified as baseline_utils
...
def parse_arguments() -> Any:
    ...
    parser.add_argument("--split_prediction",
                        type=bool,
                        default=False,
                        help="Predict with split objectives.")
...
def parse_split(arg):
    ...
    ret_split = []
    if (len(split_args) % 2 != 0) :
        raise argparse.ArgumentTypeError("Even splits required")
    else:
        ret_split = list(map(int, split_args))
        if (sum(ret_split) != 50):
            raise argparse.ArgumentTypeError("Splits should add upto 50")
    return ret_split
    ...
    for i, (_input, target, helpers) in enumerate(train_loader):
        ...
        if global_step % 1000 == 0:
            ...
            if args.split_prediction:
                print(
                    f"Train -- Epoch:{epoch}, loss:{loss}, Split rollout:{rollout_len}")
                logger.scalar_summary(tag="Train/loss_split{}".format(rollout_len),
                                     value=loss.item(),
                                      step=epoch)
            else:
                print(
...
def split_data_dicts(
        mode: str,
        data_dict: Dict[str, Any],
        args: Any,        
):
    data_dicts = []
    for i in range(len(data_dict["{}_input".format(mode)])):
        dictn = {}
        dictn["{}_input".format(mode)] = np.array(data_dict["{}_input".format(mode)][i])
        dictn["{}_output".format(mode)] = np.array(data_dict["{}_output".format(mode)][i])
        dictn["{}_helpers".format(mode)] = data_dict["{}_helpers".format(mode)]
        data_dicts.append(dictn)
    return data_dicts
...
def main():
    ...
    if not args.test:
        ...
        if args.split_prediction:
            train_loader = []
            val_loader = []
            # split the train loader data
            train_datasets = split_data_dicts("train", data_dict, args)
            for dataset_dict in train_datasets:
                train_dataset = LSTMDataset(dataset_dict, args, "train")
            ...
            # split the val loader data
            val_datasets = split_data_dicts("val", data_dict, args)
            for dataset_dict in val_datasets:
                val_dataset = LSTMDataset(dataset_dict, args, "val")
                val_loaders.append(
                    torch.utils.data.DataLoader(
                        val_dataset,
        ...
        decrement_counter = 0
        avg_decrement_counter = 0
        epoch = start_epoch

        if args.split_prediction: 
            global_start_time = time.time()
            logger = Logger(log_dir, name="split_pred")
            best_loss = float("inf")
            prev_avg_loss = best_loss
            while epoch < args.end_epoch:
                ...
                if epoch % 5 == 0:
                    ...
                    avg_loss /= len(val_loaders)
                    print(
                        f"Validation completed in {(end - start) / 60.0} mins, Total time: {(end - global_start_time) / 60.0} mins, Average loss: {avg_loss} at epoch: {epoch}"
                    )
                    ...
                    logger.scalar_summary(tag="Val/loss", value=avg_loss.item(), step=epoch)
                    ...
                    if avg_loss < prev_avg_loss:
                        ...
                        avg_decrement_counter += 1
                    prev_avg_loss = avg_loss
                    if avg_decrement_counter > 6:
                        break
        else:
