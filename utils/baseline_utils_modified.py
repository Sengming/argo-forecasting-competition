...
def get_data(args: Any, baseline_key: str
    ...
    if args.train_features:
        ...
        train_input, train_output, train_df = load_and_preprocess_data_modified(
            input_features,
            output_features,
            args,
            args.train_features,
        ...
        if args.split_prediction:
            print("Num train chunks: {}".format(len(train_input)))
            print("Num train examples per chunk: {}".format(len(train_input[0])))
            print("Prediction split ratio: {}".format(args.split_ratio))
            assert (len(input_features) == len(output_features)), "Incorrect splits, number of splits should be even"
        else:
    ...
    if args.val_features:
        ...
        val_input, val_output, val_df = load_and_preprocess_data_modified(
            input_features,
            output_features,
            args,
            args.val_features,
            mode="val")
        if args.split_prediction:
            print("Num val chunks: {}".format(len(val_input)))
            print("Num val examples per chunk: {}".format(len(val_input[0])))
            print("Prediction split ratio: {}".format(args.split_ratio))
            assert (len(input_features) == len(output_features)), "Incorrect splits, number of splits should be even"
        else:
...
def load_and_preprocess_data_modified(
        input_features: List[str],
        output_features: List[str],
        args: Any,
        feature_file: str,
    ...
    if args.split_prediction:
        __input = []
        for i in range(0, len(args.split_ratio), 2):
            if i == 0:
                __input.append(input_features_data[:, :args.split_ratio[i]])
            else:
                __input.append(input_features_data[:, sum(args.split_ratio[:i]):sum(args.split_ratio[:i+1])])
        #_input = np.array(__input)
        _input = __input
    else:
        _input = input_features_data[:, :args.obs_len]
    ...
    if mode == "test":
        ...
        if args.split_prediction:
            __output = []
            for i in range(1, len(args.split_ratio), 2):
                __output.append(output_features_data[:, sum(args.split_ratio[:i]):sum(args.split_ratio[:i+1])])
            #_output = np.array(__output)
            _output = __output
        else:
            _output = output_features_data[:, args.obs_len:]
